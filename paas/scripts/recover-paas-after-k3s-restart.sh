...
bash "${SCRIPT_DIR}/patch-frontend-wait-postgres-lab.sh" || true

echo "==> Start frontend (init container waits for postgres on every pod start)"
if kubectl get deployment frontend -n "${PAAS_NS}" >/dev/null 2>&1; then
...
echo ""
echo "Install once so this runs automatically after every reboot:"
echo "  cd ~/devsecops_paas_miscroservices && sudo bash paas/scripts/install-paas-autostart-lab.sh"
