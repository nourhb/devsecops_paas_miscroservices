...
echo ""
echo "On every boot (after k3s starts):"
echo "  - deploy/wait Postgres (postgres.paas.svc.cluster.local)"
echo "  - prisma schema check, sync frontend env, health check"
echo ""
echo "Test now (no reboot):"
echo "  sudo systemctl start paas-lab-recover"
echo "  journalctl -u paas-lab-recover -n 80 --no-pager"
echo "  bash paas/scripts/check-paas-lab-health.sh"
echo ""
echo "After reboot: wait 2–5 min, then http://192.168.56.129:30100/login"
echo "If login still shows Prisma DB error: journalctl -u paas-lab-recover -e"
