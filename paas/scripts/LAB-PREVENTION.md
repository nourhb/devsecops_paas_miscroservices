...
| **`npx next build --no-lint` / unknown option** on Next 16 | Jenkins job or PaaS sync still has **old Step 6**; `JENKINS_SYNC_INLINE_JOB_BEFORE_TRIGGER=true` pushes stale `/app/paas-bundled` from an old frontend image | `git pull` → `bash paas/scripts/fix-jenkins-paas-deploy-pipeline-lab.sh` → new build must log `crane-next16-202605`. Set `JENKINS_SYNC_INLINE_JOB_BEFORE_TRIGGER=false` in k8s env, or run `sync-paas-jenkinsfile-configmap-k8s.sh` after every Jenkinsfile change |
