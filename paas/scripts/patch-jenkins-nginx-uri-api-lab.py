#!/usr/bin/env python3
"""Emergency fix: escape Groovy $uri in Jenkins paas-deploy script OR push full Jenkinsfile."""
...
def patch_script(script: str, full_groovy: str | None) -> tuple[str, str]:
    if NGINX_MARKER in script and "writeNginxPaasDefaultConf" in script:
        return script, "already fixed"
    if full_groovy and NGINX_MARKER in full_groovy:
        return full_groovy, "replaced with full Jenkinsfile"
    if "try_files $uri $uri/" in script:
        return script.replace("try_files $uri $uri/", r"try_files \$uri \$uri/"), "escaped \$uri in try_files"
    if "try_files $uri/" in script:
        return script.replace("try_files $uri/", r"try_files \$uri/"), "escaped \$uri in try_files"
    raise SystemExit(
        "ERROR: Jenkins script has no writeNginxPaasDefaultConf and no try_files $uri pattern to patch.\n"
...
def main() -> int:
    ...
    if jenkinsfile.is_file() and NGINX_MARKER in jenkinsfile.read_text(encoding="utf-8"):
        full_groovy = jenkinsfile.read_text(encoding="utf-8").replace("\r\n", "\n")
    ...
    if new_script == old_script:
        print(f"OK: Jenkins job already has nginx uri fix ({reason})")
        return 0
    ...
    if (
        NGINX_MARKER not in script
        and r"try_files \$uri" not in script
        and "writeNginxPaasDefaultConf" not in script
    ):
        print("ERROR: verify failed — nginx uri fix not present after POST", file=sys.stderr)
        return 1

    print("OK: Jenkins paas-deploy script patched — trigger Build with Parameters (not Replay)")
