...
if kubectl get deployment frontend -n "${PAAS_NS}" >/dev/null 2>&1; then
  ...
  if [[ -n "${NS_JSON}" && "${NS_JSON}" != "{}" ]]; then
    echo "==> Remove frontend nodeSelector (${NS_JSON}) — pins block scheduling when master is control-plane"
...
if [[ -f "${ENV_FILE}" ]]; then
  PAAS_SKIP_ROLLOUT=0 ENV_FILE="${ENV_FILE}" bash "${SCRIPT_DIR}/sync-paas-frontend-env-k8s.sh" || true
else
