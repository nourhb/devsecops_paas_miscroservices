...
JENKINS_PAAS_DIR = "/var/jenkins_home/paas"
JENKINS_STAGES_REMOTE_PATH = f"{JENKINS_PAAS_DIR}/paas-deploy-stages.groovy"
JENKINS_LOAD_H1 = f"{JENKINS_PAAS_DIR}/paas-deploy-load-h1.groovy"
JENKINS_LOAD_H2 = f"{JENKINS_PAAS_DIR}/paas-deploy-load-h2.groovy"
JENKINS_LOAD_H3 = f"{JENKINS_PAAS_DIR}/paas-deploy-load-h3.groovy"
JENKINS_STAGES_VARS = f"{JENKINS_PAAS_DIR}/paas-deploy-stages-vars.groovy"
JENKINS_STAGES_P1 = f"{JENKINS_PAAS_DIR}/paas-deploy-stages-p1.groovy"
JENKINS_STAGES_P2 = f"{JENKINS_PAAS_DIR}/paas-deploy-stages-p2.groovy"
JENKINS_STAGES_P3 = f"{JENKINS_PAAS_DIR}/paas-deploy-stages-p3.groovy"
DT_STAGES_MARKER = "helm-portable-20260620-cps-split"
LEGACY_DT_STAGES_MARKER = "helm-portable-20260619"
PAAS_DEPLOY_STAGES_LOAD_MARKER = "paas-deploy-stages-load-20260620-cps-split"
...
def verify_job_script_markers(cfg_xml: str) -> bool:
    if "load paasDeployStages" in cfg_xml and "paas.runPaasDeploy()" in cfg_xml:
        return True
    if PAAS_DEPLOY_STAGES_LOAD_MARKER in cfg_xml and "load paasDeployStages" in cfg_xml:
        return True
    if "load paasLoadH1" in cfg_xml or "load paasStagesP3" in cfg_xml:
        return True
    return False
def build_load_wrapper() -> str:
    sys.path.insert(0, str(Path(__file__).resolve().parent))
    from jenkins_merge_cps_wrapper import build_wrapper

    return build_wrapper(
        paas_dir=JENKINS_PAAS_DIR,
        marker=PAAS_DEPLOY_STAGES_LOAD_MARKER,
        bundle=DT_STAGES_MARKER,
    )
def assert_jenkinsfile_crane_fix(groovy: str, path: Path) -> None:
    if "dockerlessImagePushCraneNode6a" not in groovy and not any(m in groovy for m in CRANE_MARKERS):
        print(
...
def assert_jenkinsfile_env_loader_fix(groovy: str, path: Path) -> None:
    if 'Do not use ". ./.env"' not in groovy and ENV_SAFE_DOTENV_LOADER_MARKER not in groovy:
        print(
            f"ERROR: {path} missing {ENV_SAFE_DOTENV_LOADER_MARKER} "
            "(build fails when EMAIL_PASS has spaces ÔÇö old . ./.env loader).\n"
            "  git pull origin main\n"
            "  bash paas/scripts/lab.sh jenkins",
            file=sys.stderr,
        )
...
def assert_jenkinsfile_cosign_digest_fix(groovy: str, path: Path) -> None:
    if "cosignSignImageShellSnippet" not in groovy and COSIGN_DIGEST_MARKER not in groovy:
        print(
...
def assert_jenkinsfile_sonar_step5_fix(groovy: str, path: Path) -> None:
    ...
    if "LOG=/tmp/sonar-scanner" in groovy:
        ...
        sys.exit(1)
    if "sonar.token=" not in groovy and SONAR_LOGIN_MARKER not in groovy:
        print(
            f"ERROR: {path} missing SonarScanner token auth.\n"
            "  git pull origin main\n"
            "  bash paas/scripts/lab.sh jenkins",
            file=sys.stderr,
        )
...
ENV_PARAM_DEFAULTS: dict[str, str] = {
    ...
    "SONAR_ADMIN_NEW_PASSWORD": "SONAR_ADMIN_NEW_PASSWORD",
    ...
    "PAAS_DT_UPLOAD_OPTIONAL": "PAAS_DT_UPLOAD_OPTIONAL",
...
FULL_PARAMETER_DEFINITIONS: list[tuple[str, str]] = [
    ...
    ("SONAR_ADMIN_PASSWORD", ""),
    ("SONAR_ADMIN_NEW_PASSWORD", "SonarQube123!"),
    ("SONAR_TOKEN_NAME", "paas-jenkins-lab"),
    ("DEPENDENCY_TRACK_BASE_URL", ""),
    ("DEPENDENCY_TRACK_API_KEY", ""),
    ("JENKINS_DEPENDENCY_TRACK_BASE_URL", ""),
    ("PAAS_DT_UPLOAD_OPTIONAL", "true"),
    ("NVD_API_KEY", ""),
    ("ZAP_TARGET_URL", ""),
    ...
    ("JENKINS_NEXT_BUILD_WEBPACK", "true"),
...
def merge_missing_parameter_definitions(existing_xml: str) -> str:
    if "hudson.model.ParametersDefinitionProperty" not in existing_xml:
        ...
        print("WARN: job config has no <properties> block ÔÇö cannot inject parameters automatically")
        return existing_xml
    ...
    print(f"Adding {len(missing)} missing job parameter(s): {', '.join(n for n, _ in missing[:8])}{'ÔÇª' if len(missing) > 8 else ''}")
...
def ensure_job_parameters(existing_xml: str) -> str:
    ...
    dt_optional = (os.environ.get("PAAS_DT_UPLOAD_OPTIONAL") or "true").strip()
    out = force_job_parameter_default(out, "PAAS_DT_UPLOAD_OPTIONAL", dt_optional)
    webpack = (os.environ.get("JENKINS_NEXT_BUILD_WEBPACK") or "true").strip()
    out = force_job_parameter_default(out, "JENKINS_NEXT_BUILD_WEBPACK", webpack)
...
def merge_env_param_defaults_force(
    ...
    for param_name in force_names or FORCE_ENV_PARAM_DEFAULTS:
        ...
        if not val and param_name == "JENKINS_DEPENDENCY_TRACK_BASE_URL":
            val = (
                os.environ.get("DEPENDENCY_TRACK_BASE_URL")
                or os.environ.get("JENKINS_DEPENDENCY_TRACK_BASE_URL")
                or ""
            ).strip()
        if not val:
            continue
...
def refuse_stale_groovy_overwrite(existing_xml: str, groovy: str) -> None:
    ...
    for marker in (
        ...
        if marker in existing_xml and marker not in groovy:
            print(
                f"ERROR: refusing to overwrite Jenkins job ÔÇö source Jenkinsfile is STALE "
...
def main() -> int:
    ...
    if params_only:
        ...
        if cfg_code != 200 or not existing_cfg.strip():
            print(f"ERROR: job '{job}' not found ÔÇö create it first with --force --force-full", file=sys.stderr)
            return 1
        ...
        print(
            f"Updating job '{job}' parameter defaults only ({len(xml)} bytes) ÔÇö Pipeline script unchanged"
        )
        ...
        if verify_code == 200:
            if not verify_job_script_markers(verify_cfg):
                print(
                    f"WARN: job script missing {NGINX_CONF_WRITEFILE_MARKER} ÔÇö "
    ...
    if code == 200 and not any(m in pm_body for m in pipeline_markers):
        ...
        print(
            "Run: bash paas/scripts/lab.sh jenkins-plugins",
            file=sys.stderr,
        )
        print(
            "Then: python3 paas/scripts/lib/create_jenkins_paas_deploy_job.py --force",
            file=sys.stderr,
        )
        return 1
    ...
    if code == 200 and force:
        ...
        if force_full:
            mode = "full-document"
            print(f"Updating existing job '{job}' ({len(xml)} bytes, {mode} ÔÇö params + no concurrent builds)")
        else:
            if (
                cfg_code == 200
                ...
                if cfg_code == 200 and existing_cfg.strip():
                    print(
                        "NOTE: using full-document (merged job missing required markers ÔÇö "
        ...
        if verify_code == 200:
            if not verify_job_script_markers(verify_cfg):
                ...
                return 1
            if "load paasDeployStagesPath" in verify_cfg:
                print(
                    "ERROR: job still loads monolithic paas-deploy-stages.groovy (MethodTooLarge).\n"
                    "  Re-run: bash paas/scripts/lab.sh force-fix-paas-deploy",
                    file=sys.stderr,
                )
                return 1
            if "load paasStagesP3" in verify_cfg and PAAS_DEPLOY_STAGES_LOAD_MARKER in verify_cfg:
                print(f"OK: job script uses 7-file CPS split ({PAAS_DEPLOY_STAGES_LOAD_MARKER})")
            elif PAAS_DEPLOY_STAGES_LOAD_MARKER in verify_cfg:
                print(f"OK: job script loads stages via multi-load ({PAAS_DEPLOY_STAGES_LOAD_MARKER})")
            elif "def paas = load paasDeployStages" in verify_cfg:
                print(
                    "ERROR: job uses monolith load (MethodTooLarge) — run: bash paas/scripts/lib/force-api-jenkins-paas-deploy-now.sh",
                    file=sys.stderr,
                )
                return 1
            else:
                print("OK: job script is monolithic (def runPaasDeploy + Steps 1-12)")
            if "Stale stages bundle (missing runPaasDeploy in p3)" in verify_cfg:
                print(
                    "ERROR: LIVE job still has OLD stale check — run: "
                    "bash paas/scripts/lib/force-api-jenkins-paas-deploy-now.sh",
                    file=sys.stderr,
                )
                return 1
            if "load paasStagesP3" in verify_cfg and "runPaasDeploy()" not in verify_cfg:
                print(
                    "ERROR: job loads CPS split but missing runPaasDeploy() after loads",
                    file=sys.stderr,
                )
                return 1
            for name in (
                ...
                if job_defines_string_parameter(verify_cfg, name):
                    ...
                    print(f"WARN: job parameter {name} still missing ÔÇö re-run with --force-full", file=sys.stderr)
        print(f"OK: {base}/job/{job}/")
        return 0
