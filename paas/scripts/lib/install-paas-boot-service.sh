...
LAB_USER="${PAAS_LAB_USER:-${SUDO_USER:-${USER:-master}}}"
LAB_HOME="$(getent passwd "${LAB_USER}" | cut -d: -f6)"
REPO_DIR="${PAAS_REPO_DIR:-${LAB_HOME}/devsecops_paas_miscroservices}"
...
[[ "$(id -u)" -eq 0 ]] || die "run with sudo: sudo bash paas/scripts/lib/install-paas-boot-service.sh"
...
case "${1:-install}" in
  ...
  ;;
  status)
    systemctl status paas-lab-start.service --no-pager || true
    echo ""
