...
export async function POST(req: Request) {
  ...
  try {
    ...
    // 2-6. GitOps: clone, scaffold Helm chart, commit, push
    ...
    gitopsTempDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "gitops-provision-"),
    );
    ...
    const srcChartDir = path.join(
      gitopsTempDir,
      "apps",
      "sample-app",
    );
    const destChartDir = path.join(
      gitopsTempDir,
      "apps",
      project.name,
    );
        ...
        if (file.isDirectory()) {
          ...
          for (const nested of fs.readdirSync(srcPath)) {
            ...
            const content = fs.readFileSync(nestedSrc, "utf8")
              ...
              .replace(
                /harbor\.example\.com\/project\/image/g,
                project.registryRepo,
              );
    ...
    await execAsync(
      `git commit -m "Add app ${project.name} from DevSecOps PaaS"`,
      { cwd: gitopsTempDir },
    ).catch(() => undefined); // ignore if nothing to commit
