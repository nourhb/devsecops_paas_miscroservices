...
async function fetchClusterMetrics(): Promise<ClusterMetrics> {
  ...
  const nodeCount =
    Number(nodesResp.data.data.result?.[0]?.value?.[1] ?? 0) || 0;
  const cpuUsagePercent =
    Number(cpuResp.data.data.result?.[0]?.value?.[1] ?? 0) || 0;
  const memoryUsagePercent =
    Number(memResp.data.data.result?.[0]?.value?.[1] ?? 0) || 0;
...
async function fetchSecuritySummary(): Promise<SecuritySummary> {
  ...
  // For signed/unsigned images we approximate from severity tags
