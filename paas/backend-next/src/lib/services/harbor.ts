...
function getHarborClient() {
    ...
    if (!baseUrl || !username || !password) {
        throw new Error("Harbor configuration missing (HARBOR_URL/USERNAME/PASSWORD).");
    }
    return axios.create({
...
export async function createRepository(projectName: string, repositoryName: string): Promise<void> {
...
export async function deleteRepository(projectName: string, repositoryName: string): Promise<void> {
    const client = getHarborClient();
    try {
        await client.delete(`/projects/${encodeURIComponent(projectName)}/repositories/${encodeURIComponent(repositoryName)}`);
    }
    catch {
    }
}
export async function registerPushedImage(imageRef: string): Promise<HarborPushResult> {
    const client = getHarborClient();
    ...
    try {
        const { data } = await client.get(`/projects/${encodeURIComponent(projectName)}/repositories/${encodeURIComponent(repositoryName)}/artifacts`, {
...
export async function getImageTags(projectName: string, repositoryName: string): Promise<string[]> {
    const client = getHarborClient();
    try {
        const { data } = await client.get(`/projects/${encodeURIComponent(projectName)}/repositories/${encodeURIComponent(repositoryName)}/artifacts`, {
