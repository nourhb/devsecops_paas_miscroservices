...
export interface ArgoCdAppStatus {
    ...
    healthStatus: "Healthy" | "Degraded" | "Missing" | "Progressing" | "Suspended" | "Unknown";
    lastSyncedAt?: string;
}
...
export async function upsertApplication(spec: ArgoCdAppSpec): Promise<ArgoCdAppStatus> {
...
export async function syncApplication(appName: string): Promise<ArgoCdAppStatus> {
    const client = getArgoClient();
    await client.post(`/api/v1/applications/${appName}/sync`, {});
    return getApplicationStatus(appName);
}
export async function getApplicationStatus(appName: string): Promise<ArgoCdAppStatus> {
    const client = getArgoClient();
    const { data } = await client.get(`/api/v1/applications/${appName}`);
    const sync = data.status?.sync?.status as "Synced" | "OutOfSync" | undefined;
    const health = data.status?.health?.status as "Healthy" | "Degraded" | "Missing" | "Progressing" | "Suspended" | undefined;
    return {
