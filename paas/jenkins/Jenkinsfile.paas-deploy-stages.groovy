      ...
      if (!commandExists("docker")) {
        ...
        withEnv([
          ...
          def sonarRc = sh(script: '''#!/bin/bash
            set +e
            pick_sonar_url() {
              # Built-in Jenkins agent: NodePort first (cluster DNS often unavailable).
            ...
            for _sv in 1 2 3 4 5; do
              ...
              if printf '%s' "${VALID}" | grep -q '"valid":false'; then
                echo "[sonar] SONAR_TOKEN rejected (valid:false) — generate new token: http://${NODE_IP}:${SONAR_NODEPORT:-30900} → Security → Generate Token"
                echo "[sonar] then on master: SONAR_TOKEN=sqa_... bash paas/scripts/lab.sh pipeline-heal"
                break
              fi
              if [ "${_sv}" -lt 5 ]; then
            ...
            for _sonar_try in 1 2 3; do
              ...
              if grep -qE 'Error 500 on.*rules/search|An error has occurred\\. Please contact your administrator' "${LOG}" && [ "${_sonar_try}" -lt 3 ]; then
                echo "[sonar] WARN: Sonar rules API HTTP 500 (server overloaded — two builds scanning at once breaks the lab VM)"
                echo "[sonar] Wait 90s and retry (on master: bash paas/scripts/lab.sh sonarqube)"
                sleep 90
                SONAR_HOST_URL="$(pick_sonar_url)"
                continue
              fi
