...
export function LoginForm({ keycloakEnabled, keycloakError, postRegisterEmail, postRegisterMailConsole }: LoginFormProps) {
    ...
    useEffect(() => {
        if (keycloakError?.trim()) {
            setError(keycloakError.trim());
        }
    }, [keycloakError]);
    useEffect(() => {
    ...
    const keycloakHref = `/api/auth/keycloak/login?next=${encodeURIComponent(destination)}`;
    return (<div className="flex min-h-screen items-center justify-center px-4">
      <Card className="w-full max-w-md backdrop-blur">
        ...
        <CardContent>
          ...
          {keycloakEnabled ? (<div className="mt-6 space-y-4">
              ...
              <Button type="button" variant="outline" className="w-full" asChild>
                <Link href={keycloakHref}>Continue with Keycloak</Link>
              </Button>
            </div>) : null}
