...
export async function suggestBuildParameters(input: BuildSuggestionInput): Promise<BuildHintSuggestion> {
    ...
    const projectName = deriveProjectName(normalizeText((parsed as {
        projectName?: string;
    }).projectName) || description, currentValues.projectName);
    const buildType = deriveBuildType(normalizeText((parsed as {
        buildType?: string;
    }).buildType) || description, currentValues.buildType);
    const deliveryType = deriveDeliveryType(normalizeText((parsed as {
        deliveryType?: string;
    }).deliveryType) || description, currentValues.deliveryType);
    const reasoning = normalizeText((parsed as {
        reasoning?: string;
    }).reasoning) || fallback.reasoning;
    const buildTool = normalizeText((parsed as {
        buildTool?: string;
    }).buildTool) || fallback.buildTool;
    const rawSteps: unknown[] = Array.isArray((parsed as {
...
export async function summarizeBuildOutcome(input: BuildAnalysisInput): Promise<BuildHintAnalysis> {
    ...
    const message = normalizeText((parsed as {
        message?: string;
    }).message);
    const nextStep = normalizeText((parsed as {
        nextStep?: string;
    }).nextStep);
    const stage = normalizeText((parsed as {
        stage?: string;
    }).stage);
    return {
