...
import { resolveSecureSessionCookieFlag } from "@/server/auth/auth-service";
import { signToken } from "@/server/security/jwt";
import { ApiError, ValidationError } from "@/server/http/response";
...
const KC_OAUTH_MAX_AGE_SEC = 60 * 10;
...
export function buildKeycloakOAuthCookies(input: {
    ...
    return [
        transientCookie(KC_STATE_COOKIE, input.state, KC_OAUTH_MAX_AGE_SEC),
        transientCookie(KC_VERIFIER_COOKIE, input.codeVerifier, KC_OAUTH_MAX_AGE_SEC),
        transientCookie(KC_NEXT_COOKIE, encodeURIComponent(next), KC_OAUTH_MAX_AGE_SEC)
    ];
}
