...
export async function createProject(payload: unknown, userId: string): Promise<{
    ...
    const buildEnv = normalizeBuildEnvInput(parsed.data.buildEnv);
    const created = await prisma.project.create({
        data: {
            ...
            buildEnv: buildEnv ?? Prisma.DbNull,
            namespace,
            createdById: userId
        }
    });
...
export async function updateProjectForUser(projectId: string, payload: unknown, userId: string, role: UserRole): Promise<Project> {
    ...
    const buildEnv = buildEnvRaw === undefined ? undefined : normalizeBuildEnvInput(buildEnvRaw);
    const updated = await prisma.project.update({
        ...
        data: {
            ...
            ...(buildEnvRaw !== undefined ? { buildEnv: buildEnv ?? Prisma.DbNull } : {})
        }
    });
