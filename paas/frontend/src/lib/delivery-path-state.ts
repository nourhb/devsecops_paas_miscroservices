...
function inferActiveStageIndex(logs: string): number {
    ...
    if (/Step 12 —/i.test(logs) || /\*\*\* BEGIN :\s*14\.\s*Archivage/i.test(logs)) {
        return 3;
    }
    if (/Step 10 —/i.test(logs) || /Step 11 —/i.test(logs) || /10b\.\s*ZAP baseline/i.test(logs) || /11\.\s*Publication Helm OCI/i.test(logs)) {
        return 2;
    }
