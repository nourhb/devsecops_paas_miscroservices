/**
 * Create or update a local ADMIN user (bcrypt password, email marked verified).
 * Lab / dev only — do not use weak passwords in production.
 *
 * Usage (from paas/frontend, DATABASE_URL must reach Postgres):
 *   npm run auth:seed-admin
 *   npm run auth:seed-admin -- admin@paas.local "123456789" "Platform Admin"
 *
 * Env overrides: SEED_ADMIN_EMAIL, SEED_ADMIN_PASSWORD, SEED_ADMIN_FULL_NAME
 */
