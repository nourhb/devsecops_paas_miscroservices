...
function New-BasicAuthHeaders {
  ...
  $pair = [Convert]::ToBase64String([Text.Encoding]::ASCII.GetBytes("${Username}:${ApiToken}"))
  return @{ Authorization = "Basic $pair" }
}

$dotenvPath = Join-Path $PSScriptRoot "paas\frontend\.env"

$base = $env:JENKINS_BASE_URL
if (!$base) { $base = $env:JENKINS_URL }
if (!$base) { $base = Get-DotEnvValue -Path $dotenvPath -Key "JENKINS_BASE_URL" }
if (!$base) { $base = Get-DotEnvValue -Path $dotenvPath -Key "JENKINS_URL" }
if (!$base) { $base = "http://192.168.56.129:30376" }

$user = $env:JENKINS_USERNAME
if (!$user) { $user = Get-DotEnvValue -Path $dotenvPath -Key "JENKINS_USERNAME" }
if (!$user) { $user = "admin" }

$token = $env:JENKINS_API_TOKEN
if (!$token) { $token = Get-DotEnvValue -Path $dotenvPath -Key "JENKINS_API_TOKEN" }
if (!$token) { throw "Missing Jenkins API token. Set env:JENKINS_API_TOKEN or add JENKINS_API_TOKEN to paas/frontend/.env" }

$headers = New-BasicAuthHeaders -Username $user -ApiToken $token

$session = New-Object Microsoft.PowerShell.Commands.WebRequestSession

# Validate creds early (gives a clearer error than crumbIssuer 401)
try {
  Invoke-WebRequest -UseBasicParsing -WebSession $session -Uri "$base/api/json" -Headers $headers | Out-Null
} catch {
  throw "Jenkins auth failed for $base as user '$user'. Verify JENKINS_USERNAME + JENKINS_API_TOKEN. Underlying error: $($_.Exception.Message)"
}

$crumbResp = Invoke-WebRequest -UseBasicParsing -WebSession $session -Uri "$base/crumbIssuer/api/json" -Headers $headers
$crumb = $crumbResp.Content | ConvertFrom-Json
if (!$crumb.crumbRequestField -or !$crumb.crumb) { throw "Failed to obtain Jenkins crumb from $base/crumbIssuer/api/json" }
...
<flow-definition plugin="workflow-job@1505.vea_4b_20a_4a_495">
  ...
  <description>Shared deploy pipeline for DevSecOps PaaS</description>
...
Invoke-WebRequest `
  -UseBasicParsing `
  -WebSession $session `
  -Uri "$base/job/paas-deploy/config.xml" `
  -Method POST `
